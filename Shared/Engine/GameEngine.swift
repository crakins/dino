...
struct GameEngine {
    /// Returns true if collision occurred (caller should handle haptics)
    static func update(state: GameState, deltaTime: TimeInterval, canvasSize: CGSize) -> Bool {
        ...
        if checkCollision(state: state, canvasSize: canvasSize) {
            ...
            return true // Collision occurred - caller handles haptics
        }
